...
public extension CastClientDelegate {
    func castClient(_: CastClient, willConnectTo _: CastDevice) {}
    func castClient(_: CastClient, didConnectTo _: CastDevice) {}
    func castClient(_: CastClient, didDisconnectFrom _: CastDevice) {}
    func castClient(_: CastClient, connectionTo _: CastDevice, didFailWith _: Error?) {}
    func castClient(_: CastClient, deviceStatusDidChange _: CastStatus) {}
    func castClient(_: CastClient, mediaStatusDidChange _: CastMediaStatus) {}
}
...
public final class CastClient: NSObject, RequestDispatchable, Channelable {
        ...
        socketQueue.async {
            do {
                ...
                CFStreamCreatePairWithSocketToHost(
                    nil,
                    self.device.hostName as CFString,
                    UInt32(self.device.port),
                    &readStream,
                    &writeStream
                )
                ...
                DispatchQueue.main
                    .async { self.delegate?.castClient(self, connectionTo: self.device, didFailWith: error as NSError) }
            }
        }
    }
    ...
    func send(_ request: CastRequest, response: CastResponseHandler?) {
        ...
        do {
            let messageData = try CastMessage.encodedMessage(
    ...
    public func stopCurrentApp() {
        guard outputStream != nil, let app = currentStatus?.apps.first else {
            return
        }
    ...
    public func requestMediaStatus(
        for app: CastApp,
        completion _: ((Result<CastMediaStatus, CastError>) -> Void)? = nil
    ) {
        guard outputStream != nil else {
            return
        }
    ...
    public func seek(to currentTime: Float) {
        ...
        if let mediaStatus = currentMediaStatus {
            ...
            mediaControlChannel.requestMediaStatus(for: app) { result in
                switch result {
                case let .success(mediaStatus):
                    self.mediaControlChannel.sendSeek(
                        to: currentTime,
                        for: app,
...
extension CastClient: ReceiverControlChannelDelegate {
    func channel(_: ReceiverControlChannel, didReceive status: CastStatus) {
...
extension CastClient: MediaControlChannelDelegate {
    func channel(_: MediaControlChannel, didReceive mediaStatus: CastMediaStatus) {
...
extension CastClient: HeartbeatChannelDelegate {
    func channelDidConnect(_: HeartbeatChannel) {
        if !isConnected {
            isConnected = true
        }
    }

    func channelDidTimeout(_: HeartbeatChannel) {
        disconnect()
        currentStatus = nil
...
// MARK: MultizoneControlChannelDelegate

extension CastClient: MultizoneControlChannelDelegate {
    func channel(_: MultizoneControlChannel, added _: CastMultizoneDevice) {}

    func channel(_: MultizoneControlChannel, updated _: CastMultizoneDevice) {}

    func channel(_: MultizoneControlChannel, removed _: String) {}

    func channel(_: MultizoneControlChannel, didReceive status: CastMultizoneStatus) {
